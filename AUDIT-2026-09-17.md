...
| 2026-09-19 | 2A: shift swaps and releases | `api/views.py`, `api/serializers.py` | Close the escalation-table rows for `ShiftExchange` / `OpenShiftRequest`. | **Worse than reported.** Creating a swap never checked the requester owned the shift or that the colleague was in the same company, so an officer could offer anyone's shift — any company's — to an accomplice, whose acceptance auto-approved the reassignment. Now: own shift only, same-company target, bilateral shift must be the target's. Workflow fields (`status`, `manager_*`, `target_response`, `claimed_by`) server-controlled; default update/delete return 405 (no client used them). | `api/tests/test_p2_exchange_authz.py` (9), 7 failed pre-fix. | Done |
| 2026-09-19 | 2A: officer-writable default routes | `api/permissions.py` (new `ManagerOnlyWritesMixin`), `api/views.py`, `api/serializers.py`, `finance_integrations/views.py` + `serializers.py`, `leave_management/views.py` + `permissions.py` | Walk every ViewSet as an officer; gate or justify each open default write. | 31 found open; 17 were escalations, all closed: finance connections/mappings/exports/OAuth now manager/admin only and `created_by` read-only (it decides a connection's company); approved leave locked against the officer (was extendable, and paid per day); statutory checks and compliance violations not rewritable/deletable by officers; incident resolution fields read-only, venue/shift checked on create; slot misses, shift templates (+`apply` staff checked) manager-only; employment type and pay frequency not self-editable (both profile paths); unavailability `staff_user` pinned; terms acceptance, emergency contacts, availability, preferred venues bound to the caller's own profile/company; logbook sign-off venue checked; `PATCH /users/{id}/` no longer echoes a new password in plain text. Also found: `ReadOnlyForStaffMixin` passed a permission *class*, so every refused write to leave types/policies was a 500 — fixed. | `api/tests/test_p2_writes_authz.py` (21), 19 failed pre-fix (verified by stashing); new `WriteRouteRatchetTests` pins the remaining 52 officer-open routes, each with a reason. | Done |
| 2026-09-19 | 2A regression check | — | Full suite per file vs end of Phase 1. | Only change: +31 passing tests (`test_p2_exchange_authz.py` 9, `test_p2_writes_authz.py` 21, write-route ratchet 1). 527 passed, 167 failed, 9 errors, all standing. | Isolated-DB full run. | Verified |
| 2026-09-19 | 2E: web honesty | frontend: `payroll/components/RightRailCards.tsx`, `attendance/components/{TimesheetsView,ExceptionsView,AttendanceHeader}.tsx`, `scheduling/components/ViolationsBanner.tsx`, `integrations/components/IntegrationsHeader.tsx`, `compliance/…`, `leave/…`, `incidents/IncidentsPage.tsx`, `recruitment/…`, `Router.tsx`, `contexts/AuthContext.tsx`, new `features/auth/NotForThisRolePage.tsx` | Stop the UI stating things it doesn't know. | Payroll composition no longer shows £84,210 of fixtures when data is missing; "No SIA issues this run" is no longer shown on a failed or loading query. Five handler-less buttons are disabled with a reason; the timesheet help text no longer promises bulk approval. Compliance, leave, incidents and recruitment say "couldn't load" instead of "no violations — staff within thresholds", "you're all caught up", "every incident has been reviewed" on a failed request. Dashboard routes are admin/manager only (officers get a page pointing to the app). The query cache is cleared at login and logout — its keys carry no user or company, so a shared machine showed the previous user's data for up to 10 minutes. **Found:** ordinary managers get 403 on the leave approval queue (backend, Phase 2C); the frontend has no onboarding UI, so self-serve signup cannot create a company (Phase 3). | `npm run build` clean (includes `tsc`); biome unchanged at 337. No frontend test suite exists. | Done |
| 2026-09-19 | 2B: trustworthy tests | 12 test files only | Repair fixture drift so the standing failures mean something. | 176 failures+errors → 48 failures, 0 errors; +12 principled skips (dead 'optimized' managers never installed, psutil, one deliberately removed balance block). No product code changed. Every remaining failure is a product bug or decision: leave permissions ignore `User.role` (19), regional compliance 500s on a missing manager method with a latent cross-tenant write behind it (16), compliance metrics serializer lists 10 non-existent fields (4), onboarding returns third-party secrets to managers (1, security), and smaller items. | Full suite: 643 passed, 48 failed, 0 errors, 13 skipped; only the 12 repaired files changed. | Done |
| 2026-09-19 | 2C (part 1): bugs 2B surfaced | `api/serializers.py`, `api/views.py`, `leave_management/permissions.py` + `views.py`, tests, `docs/audit-2026-09-17-prod-checks.sql` | Fix E1, A, C, D; guard B. | **E1 (security):** onboarding returned third-party credentials to any member via `step_data`; secrets now go only to `credentials` (no serializer exposes it), output is redacted, and a prod check lists companies whose credentials must be rotated. **A:** leave permissions read a role field that doesn't exist, so tenant managers got 403 on approvals, team overview and reports; now read `User.role`. Global leave config (types, policies, SystemConfig, global blackout periods) stays platform-staff only, made explicit; the ratchet then caught `LeaveBalanceViewSet` returning every company's balances to admins once they were recognised — scoped, as is `summary`. Leave export 500 fixed. **C:** compliance metrics serializer named 10 fields the model lacks (500 on any data) — mapped or dropped. **D:** bulk-resolve opened to tenant managers/admins like single resolve; `recalculate` (a no-op that claimed "initiated") returns 501. **B:** regional compliance still 500s (Phase 3), but apply-preset is platform-only and schedule validation / region detection are company-scoped *before* the repair can expose them; fake `regional-settings` saves return 501. | Full suite: 48 → 22 failures (16 regional feature bugs, 3 onboarding, 3 recruitment); leave 5→26 passing, compliance 34→40. Four regional tests realigned from "500"/fake-success to the intended 403/501. | Done |
| 2026-09-19 | 2C (part 2): money integrity | `api/models.py` + `0076_protect_pay_history`, `api/exception_handlers.py`, `core/settings.py`, `api/tasks.py`, `backfill_payable_hours`, `report_ot_basis_delta`, `api/serializers_billing.py`, `finance_integrations/services.py` + `views.py`, duplicate-clean-up scripts, web invoices/venues/scheduling | `InvoiceItem.shift` CASCADE deleted pay lines (10 invoices, GBP 1,304.80); webhook pays unapproved invoices, no `paid_date`, no replay protection; `payable_hours` never backfilled; "outstanding" defined three ways; nightly purge NameError | Pay-history FKs are `PROTECT` (no-op SQL); a `ProtectedError` returns 409 with what's in the way. Webhook settles only `approved` invoices, sets `paid_date`, and ignores replayed event ids. Backfill command (dry run by default, NULL rows only). One "outstanding" definition matching the Outbox. Purge returns a summary. **New finding:** the webhook route can never pass its own two signature checks and doesn't parse Xero's real payload, so it has never processed a payment. | Reproductions failed first (8 of 10 delete/purge tests; 4 of 6 webhook tests with the fix stashed). Full suite 684 passed / 22 failed, no per-file regressions. | Done |
| 2026-09-19 | 2D: would we know | `core/health.py`, `core/urls.py`, `api/checks.py`, `api/apps.py`, `api/tasks.py`, `core/settings.py`, `core/celery_app.py`, `docker/docker-compose.yml` | Health check cannot fail; no alert when payroll doesn't run; beat scheduler differs between dev and prod; the `celery_app.py` beat schedule is discarded | Liveness unchanged; readiness at `/api/v1/health/ready/` (Postgres + Redis, 503 when down, no error text returned). Daily `check_payroll_runs_exist` logs an ERROR per missing run. System checks: DEBUG on Render is an Error (fails `migrate`, so fails the deploy); open signup, no Sentry, non-durable storage are Warnings. `DatabaseScheduler` everywhere; the dead schedule was removed and its two never-run clean-up jobs listed for a decision. **New findings:** `core/settings/production.py` is unreachable; Sentry is initialised twice, once with `send_default_pii=True`. | `api/tests/test_p2_ops.py` 11 passed; the guard set is 28 files, 375 passed. | Done |
| 2026-09-19 | 2F (server side): minimum app build | `api/middleware/app_version.py`, `core/settings.py`, `mobile/app.config.js`, `api/tests/test_p2_app_version_gate.py`, CI guard list | Old builds with a known defect run forever; no server-side way to retire them | Builds that send `X-App-Platform`/`X-App-Build` below `MIN_APP_BUILD_{IOS,ANDROID}` get 426 `app_update_required` (health checks exempt). Off (0) by default. Requests without the headers pass: the web admin and pre-gate builds, whose offline queue would swallow a refused check-in. Build bumped 15 → 16 so the fixed build is distinguishable. | 2 of 7 tests failed before the middleware (the two refusals); 7/7 after. Mobile Jest 61 passing. | Done |
| 2026-09-19 | Test hygiene | `api/tests/test_recruitment_conversion.py` | `logging.disable(logging.CRITICAL)` at import ran at pytest collection and silenced logging for every test collected after it, so `assertLogs` in `test_p2_ops.py` failed only in a full run | Moved into `setUpModule` / `tearDownModule`. | Full suite 715 passed / 22 failed / 13 skipped (the 22 known: 16 regional compliance, 3 onboarding, 3 recruitment). Guard set 29 files, 384 passed. | Done |
