...
class ShiftScheduleDigestTests(APITestCase):
    ...
    def test_single_shift_create_still_sends_per_shift_email(self):
        ...
        self.assertEqual(call_kwargs["user_id"], self.staff_a.id)
    ...
    def test_bulk_publish_digest_suppresses_per_shift_channels(self):
        # publish_shifts on N drafts should yield exactly one digest email
        # and zero per-shift push/in-app notifications.
    ...
    def test_bulk_create_publish_suppresses_per_shift_channels(self):
        # bulk_create with is_published + send_notifications must hit the
        # digest path and skip the per-shift signal channels.
        from api.models import Notification
