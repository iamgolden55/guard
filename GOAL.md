...
| 2026-09-07 | P1-2 | `shifts/services.py`, `api/models.py` | Append `actual_hours_worked` when a timestamp is written; honour explicit hours; guard approval with no hours. | **Primary claim did not reproduce.** Hours persist (8.00) — the TimeAdjustment sync signal lists the column in its own `update_fields`, so `Shift.save()`'s recomputation lands. Correct by coincidence in another module. Listed it where the timestamp is written anyway; fixed the two defects that *do* reproduce. | `shifts/test_overnight_attendance.py::TwoStepAttendanceTests` (6 tests); service-level probe run directly. | Done, finding corrected |
| 2026-09-07 | P1-1 | `api/models.py`, `api/views.py`, migration `0069` | Partial unique constraint, idempotent generation, lock `payroll_generate`. | As intended. `IntegrityError` returns the winning row instead of raising. Per-officer `select_for_update`, not whole-loop. | `api/tests/test_invoice_concurrency.py::InvoiceGenerationConcurrencyTests` (4 tests) — the threaded race produced 2 invoices pre-fix. | Done |
| 2026-09-07 | P1-4 | `api/models.py`, `shifts/views.py`, migration `0070` | Four indexes; `ExclusionConstraint` on overlapping assignments; re-check inside `bulk_create`'s transaction. | As intended, plus `BtreeGistExtension()` in the migration. `bulk_create` catches the violation per slot and downgrades to an open slot rather than 500-ing the batch. Three existing test fixtures encoded overlapping assignments for one officer and were corrected. | `api/tests/test_invoice_concurrency.py::ShiftOverlapConstraintTests` (5 tests). | Done |
| 2026-09-07 | P1-5 | `api/models.py`, `api/signals.py`, `shifts/views.py` | Extend the lock past `paid`; make refusal visible. | As intended. `Invoice.is_locked()` covers `approved`/`paid` and an exported payroll run; the endpoint returns 409 `invoice_locked` instead of silently returning 201. | `api/tests/test_invoice_lock.py` (4 tests). | Done |
| 2026-09-07 | P1-7 | `leave_management/views.py` | Sweep the module, not the three cited lines. | As intended. Three shared helpers; nine call sites scoped. Also found and fixed `LeaveCalendarViewSet`, which rendered every approved leave request on the platform to any manager — not in the original finding. | `api/tests/test_tenant_isolation.py` (6 tests). | Done |
| 2026-09-07 | P1-8 | `api/views.py`, `api/urls.py`, `core/urls.py` | Authenticated serving, UUID names, correct the media comment. | As intended. Per-owner directories so authorisation works before the licence row exists; legacy flat paths resolve their owner through the licence and otherwise fail closed. S3-on-Render remains unverified. | `api/tests/test_sia_document_access.py` (10 tests). | Done |
| 2026-09-07 | P1-6 | `api/middleware/tenant_middleware.py`, `api/views.py`, `api/views_billing.py`, `shifts/views.py`, `leave_management/views.py` | DRF-level resolver honouring `X-Company-ID`; collapse the duplicated helpers; correct the misleading comments. | As intended. `resolve_request_company` now reads and validates the header, caches per request, and raises `CompanyAccessDenied` (a DRF `PermissionDenied`, so 403) rather than silently falling back. 24 copies in `api/views.py` plus 8 inline reads in `shifts/views.py` and the billing helper now delegate to it. Also found the Deputy viewsets and several shift endpoints read `request.current_company` directly with an empty-result fallback — so they returned nothing to every API caller. | `api/tests/test_company_switching.py` (6 tests) — 4 failed pre-fix; `X-Company-ID: A` returned B's data. | Done |
| 2026-09-07 | P1-3 + P3-3 | `api/models.py`, `core/settings.py`, migration `0071`, new `report_ot_basis_delta` command | Persist `payable_hours`; align the OT accumulator and invoice header to it, behind `OT_BASIS_ALIGNED` defaulting False. | As intended, per D2. Extracted `compute_payable_hours()` so payment, column and accumulator share one definition. Flag off = byte-identical (`test_payroll_math.py` 25/25 unchanged). | `api/tests/test_ot_basis.py` (10 tests) — asserts both flag states, including the pre-fix behaviour pinned with the flag off. | **Built, disarmed** |
| 2026-09-07 | P2-4 | `api/serializers.py`, `api/views.py` | Server-set `timestamp` and `venue_capacity`; validate shift ownership; apply to `FireExitCheck` and `ToiletCheck`. | As intended. Ownership rule lives on `CompanyScopedCheckMixin` so all three share it. Cross-tenant injection confirmed live pre-fix (201). | `api/tests/test_statutory_check_integrity.py` (10 tests). | Done |
| 2026-09-07 | P2-8 | `shifts/views.py`, `mobile/.../ShiftDetailsScreenV2.tsx` | Server-side end-time bound; align the client constant. | As intended. `LATE_CHECKIN_GRACE` = 30 min, shared shape on both sides. The client previously cut off at `end_time` exactly, refusing genuinely late arrivals the server would take. | `shifts/test_checkin_window.py` (4 tests incl. overnight regression). | Done |
| 2026-09-07 | P2-1 | `shifts/views.py` | Check SIA eligibility at check-in — allow, alert, flag (D3). | As intended. Validity measured on the *shift date*, not today. Never blocks; failure to evaluate is swallowed so an alert can never strand an officer. | `shifts/test_checkin_window.py` (4 tests). | Done |
| 2026-09-07 | P2-3 | `api/models.py` | Haversine primary; Maps call must never deny. | Went further: the Maps call is removed from the decision entirely. A geofence is a radius, not a walking route, and the call also cost money on every check-in and check-out. | `shifts/test_checkin_window.py::GeofenceFallbackTests` (5 tests incl. `ZERO_RESULTS`). | Done |
| 2026-09-07 | P2-6 | `api/models.py`, migration `0072` | Partial unique on open requests; lock in `approve_claim`. | As intended. Also locks the *shift* row, since that is what approval writes. | `api/tests/test_shift_audit_trail.py::OpenShiftRequestRaceTests` (4 tests). | Done |
| 2026-09-07 | P2-5 + P-M3 | `api/models.py`, `shifts/views.py`, `frontend/.../shiftService.ts`, `mobile/.../locationService.ts` + screens | Transmit and persist `accuracy` and `mocked`; flag rather than block. | As intended, in the existing location JSON — no migration. Server derives `low_accuracy` against a 100 m threshold. | Covered by the check-in tests; no new column to assert. | Done |
| 2026-09-07 | P2-7 | `api/middleware/audit_context.py` (new), `api/signals.py`, `core/settings.py` | Cover the shift lifecycle with actor/action/target/before/after/IP/UA. | As intended. Needed a request-context middleware (`asgiref.local.Local`, not a thread-local — the API is ASGI) for the actor. Batch-created shifts are excluded so a 50-shift batch does not produce 50 rows. | `api/tests/test_shift_audit_trail.py` (9 tests, incl. one asserting no signature or photo reaches the log). | Done |
| 2026-09-07 | P-M2 | `shifts/serializers.py`, mobile screens, `mobile/src/utils/constants.ts` | Pass `venue.check_radius` through; delete the dead constant. | As intended. `check_radius` was not on the shift payload at all and had to be added to `SimpleVenueSerializer`. | Covered by the geofence tests; mobile has no test for this screen. | Done |
| 2026-09-07 | P-M5 | `mobile/src/services/locationService.ts` | 15 s timeout; drop the inert options. | As intended. | Mobile Jest unchanged vs main. | Done |
| 2026-09-08 | P-M1 | `api/models.py`, `shifts/views.py`, migration `0073`, `mobile/src/services/syncService.ts`, `mobile/src/hooks/useNetworkStatus.ts` | Accept `occurred_at` on a flagged replay into a distinct column; relax the date window for replays; surface failed queue items. | As intended. `reported_check_in_time` / `reported_check_out_time` / `needs_attendance_review` added; pay keeps reading the server-stamped column. A permanent queue failure now raises a non-dismissable alert naming the shift and telling the officer to contact their manager. | No mobile test harness for this path; backend suite unchanged. | Done |
| 2026-09-08 | P-M1b | `mobile/src/services/syncService.ts`, `api.config.ts`, `shiftsSlice.ts`, new `report_unrecorded_breaks` command | Delete the dead break plumbing; add a metric; do **not** populate `break_duration` (D5). | As intended. Sync action types, endpoint constants and Redux reducers removed. The command reports shifts over the regulation trigger with no break recorded; it writes nothing. | Command runs (0 rows — dev DB was reset by the fresh-migration gate). | Done |
| 2026-09-08 | P-M1c | 7 screen files + `screens/dashboard/components/` | Delete the unrouted v1 screens. | **4,633 lines of screens plus 2,721 of orphaned components = 7,354.** Every file verified unimported (not merely unrouted) before deletion. Removed 130 pre-existing type errors as a side effect. | `tsc` 677 → 539 errors, no new ones; Jest identical to main. | Done |
| 2026-09-08 | P-M4 | `shifts/views.py`, `mobile/src/services/syncService.ts` | Machine-readable `code`; match on it, never prose. | As intended. `already_checked_in`, `already_checked_out`, `not_checked_in` added on both viewsets. Prose match kept as a fallback for older backends. | Backend suite unchanged. | Done |
| 2026-09-08 | P-M6 | `mobile/src/services/shiftsService.ts` | Delete the dead hyphenated methods. | As intended — 52 lines. Confirmed zero callers first. | `tsc` clean on the file. | Done |
| 2026-09-08 | P-M7 | 28 mobile files | Route logs through the existing `logger`. | 238 `console.*` calls rewritten; mobile now has zero outside the logger itself and its tests. Two multi-line import blocks were broken by the sweep and fixed. Frontend's 67 remain — see P3-13. | `tsc` no new errors; Jest identical. | Done |
| 2026-09-08 | P3-1 | `api/views.py`, `core/settings.py` | Invite gate behind a setting defaulting to current behaviour. | As intended. `REGISTRATION_REQUIRES_INVITE`, default False. | Existing suite unchanged. | **Built, disarmed** |
| 2026-09-08 | P3-2 | new `api/tests/test_serializer_mass_assignment.py` | Sweep the remaining `__all__` serialisers. | **Ratchet instead of sweep.** The two that mattered (Invoice, SIALicense) plus the three check serialisers are explicitly fielded. The other 26 were each inspected: every one is attached to a ViewSet that gates writes by role, queryset or `perform_*`, so rewriting them is churn with real regression risk and no proven exposure. A test now pins the set, so a *new* `__all__` fails and has to be argued for. | `test_serializer_mass_assignment.py` (3 tests). | Done, scope adjusted |
| 2026-09-08 | P3-4 | `api/models.py` | Quantise at one boundary, `ROUND_HALF_UP`. | **Not applied.** Quantising hours before multiplying changes what people are paid — 7.6666… h at £15 stores £115.00, 7.67 h stores £115.05. Pennies, but pennies in wages, so Rule 1 applies. Documented in place with the exact effect. | — | **Deferred** |
| 2026-09-08 | P3-5 | `leave_management/views.py` | Validate `country_code` / `year` before the outbound path and the cache key. | As intended. ISO-3166 alpha-2 and a 2000–2100 year range. | Existing suite unchanged. | Done |
| 2026-09-08 | P3-6 | `frontend/src/services/shiftService.ts` | Delete or implement `startShift` / `endShift`. | Deleted — 21 lines calling two routes that exist in no router. | `tsc` + build clean. | Done |
| 2026-09-08 | P3-8 | `shifts/serializers.py` | Fix the values-queryset related field. | Made `read_only`, which is what the shim now is. The queryset could never have resolved a FK. | `tsc` n/a; backend suite unchanged. | Done |
| 2026-09-08 | P3-9 | new `frontend/src/lib/apiError.ts`, attendance hook + page | Surface a conflict state and refetch. | As intended, wired where a 409 now actually exists (`invoice_locked` from P1-5). Shared helper replaces two private copies of `extractApiError` that knew nothing about 409. | `tsc` + build clean. | Done |
| 2026-09-08 | P3-10 | — | Add `invalidateQueries` to the attendance feature. | **Did not reproduce.** `useAttendanceData.ts` already invalidates live, timesheets and adjustments on every mutation's `onSettled`. The audit's "zero occurrences" was wrong. | Read the file. | Not needed |
| 2026-09-08 | P3-11 | `api/models.py` | Stop `Shift.clean()` querying on every save. | Availability and leave validation now runs only when the assignment is created or moved. Also fixes a live bug the audit did not name: leave approved *after* a shift was scheduled made that shift unclosable, because check-out re-ran a scheduling rule against changed data. | `shifts/` + audit suite, 104 passed. | Done |
| 2026-09-08 | P3-12 | `requirements.txt`, `requirements/base.txt` | `pip-audit`; patch versions only. | Django 5.2→5.2.11, cryptography 43.0.0→43.0.1, requests 2.32.0→2.32.4, sqlparse 0.5.3→0.5.4. **163 vulnerabilities → 116.** The rest need minor/major bumps and are reported, not applied. | Image rebuilt; full suite per-file identical. | Done |
| 2026-09-08 | P3-13 | new `frontend/src/lib/logger.ts`, 18 files | Strip or route `console.log`. | 250 `console.*` calls routed through a new logger that is dev-only for debug and forwards warn/error to the existing `monitoring` stub — so they start reporting when Sentry is re-enabled. Zero raw calls remain. | `tsc` + build clean, lint unchanged at 337. | Done |
| 2026-09-08 | P3-14 | `api/consumers.py` | Remove or scope the global notification group. | Removed. Nothing has ever broadcast to it, so nothing leaked; any future send would have been cross-tenant by construction. | Backend suite unchanged. | Done |
| 2026-09-08 | P3-15 | `core/settings.py` | Tidy the LAN IPs out of `ALLOWED_HOSTS`. | As intended. Every environment sets `DJANGO_ALLOWED_HOSTS`, so no behaviour change. | — | Done |
| 2026-09-08 | P3-16 | `api/views.py` | Stop login 500-ing on a duplicate email. | As intended. Exact username wins, else the earliest matching email. `email` is unique in the serialiser but not in the database. | Backend suite unchanged. | Done |
